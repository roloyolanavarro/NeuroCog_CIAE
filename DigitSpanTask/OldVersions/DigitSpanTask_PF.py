...
expName = 'DigitSpanTask_PF'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\INU\\Documents\\GitHub\\NeuroCog_CIAE\\DigitSpanTask\\digit-span-visual\\DigitSpanTask_PF.py',
...
drift_trainPF = visual.ImageStim(
    win=win,
    name='drift_trainPF', 
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
start_trainPF = sound.Sound('datasets/microwave-beep.wav', secs=0.5, stereo=True, hamming=True,
    name='start_trainPF')
start_trainPF.setVolume(1.0)
...
driftPF = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
startPF = sound.Sound('datasets/microwave-beep.wav', secs=0.5, stereo=True, hamming=True,
    name='startPF')
startPF.setVolume(1.0)
...
instrucloop_PF = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('datasets/instructions.xlsx'),
    seed=None, name='instrucloop_PF')
thisExp.addLoop(instrucloop_PF)  # add the loop to the experiment
thisInstrucloop_PF = instrucloop_PF.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisInstrucloop_PF.rgb)
if thisInstrucloop_PF != None:
    for paramName in thisInstrucloop_PF:
        exec('{} = thisInstrucloop_PF[paramName]'.format(paramName))

for thisInstrucloop_PF in instrucloop_PF:
    currentLoop = instrucloop_PF
    # abbreviate parameter names if possible (e.g. rgb = thisInstrucloop_PF.rgb)
    if thisInstrucloop_PF != None:
        for paramName in thisInstrucloop_PF:
            exec('{} = thisInstrucloop_PF[paramName]'.format(paramName))
...
# completed 1.0 repeats of 'instrucloop_PF'


# set up handler to look after randomisation of conditions etc
trial_trainingPF = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('datasets/training_pf.xlsx'),
    seed=None, name='trial_trainingPF')
thisExp.addLoop(trial_trainingPF)  # add the loop to the experiment
thisTrial_trainingPF = trial_trainingPF.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrial_trainingPF.rgb)
if thisTrial_trainingPF != None:
    for paramName in thisTrial_trainingPF:
        exec('{} = thisTrial_trainingPF[paramName]'.format(paramName))

for thisTrial_trainingPF in trial_trainingPF:
    currentLoop = trial_trainingPF
    # abbreviate parameter names if possible (e.g. rgb = thisTrial_trainingPF.rgb)
    if thisTrial_trainingPF != None:
        for paramName in thisTrial_trainingPF:
            exec('{} = thisTrial_trainingPF[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    start_trainPF.setSound('datasets/microwave-beep.wav', secs=0.5, hamming=True)
    start_trainPF.setVolume(1.0, log=False)
    firstfixtrain.setText('+')
    # keep track of which components have finished
    firstfix_trainingComponents = [drift_trainPF, start_trainPF, firstfixtrain]
    ...
    while continueRoutine:
        ...
        # *drift_trainPF* updates
        if drift_trainPF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            drift_trainPF.frameNStart = frameN  # exact frame index
            drift_trainPF.tStart = t  # local t and not account for scr refresh
            drift_trainPF.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(drift_trainPF, 'tStartRefresh')  # time at next scr refresh
            drift_trainPF.setAutoDraw(True)
        if drift_trainPF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > drift_trainPF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                drift_trainPF.tStop = t  # not accounting for scr refresh
                drift_trainPF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(drift_trainPF, 'tStopRefresh')  # time at next scr refresh
                drift_trainPF.setAutoDraw(False)
        # start/stop start_trainPF
        if start_trainPF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            start_trainPF.frameNStart = frameN  # exact frame index
            start_trainPF.tStart = t  # local t and not account for scr refresh
            start_trainPF.tStartRefresh = tThisFlipGlobal  # on global time
            start_trainPF.play(when=win)  # sync with win flip
        if start_trainPF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > start_trainPF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                start_trainPF.tStop = t  # not accounting for scr refresh
                start_trainPF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(start_trainPF, 'tStopRefresh')  # time at next scr refresh
                start_trainPF.stop()
        ...
        if firstfixtrain.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > firstfixtrain.tStartRefresh + fixation_time_trainPF-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in firstfix_trainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    start_trainPF.stop()  # ensure sound has stopped at end of routine
    trial_trainingPF.addData('firstfixtrain.started', firstfixtrain.tStartRefresh)
    trial_trainingPF.addData('firstfixtrain.stopped', firstfixtrain.tStopRefresh)
    # the Routine "firstfix_training" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    train_digitloopPF = data.TrialHandler(nReps=digitSpan_trainPF, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='train_digitloopPF')
    thisExp.addLoop(train_digitloopPF)  # add the loop to the experiment
    thisTrain_digitloopPF = train_digitloopPF.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrain_digitloopPF.rgb)
    if thisTrain_digitloopPF != None:
        for paramName in thisTrain_digitloopPF:
            exec('{} = thisTrain_digitloopPF[paramName]'.format(paramName))
    
    for thisTrain_digitloopPF in train_digitloopPF:
        currentLoop = train_digitloopPF
        # abbreviate parameter names if possible (e.g. rgb = thisTrain_digitloopPF.rgb)
        if thisTrain_digitloopPF != None:
            for paramName in thisTrain_digitloopPF:
                exec('{} = thisTrain_digitloopPF[paramName]'.format(paramName))
        ...
        # update component parameters for each repeat
        digitpresentation_training.setText(str(digits_trainPF)[train_digitloopPF.thisN])
        # keep track of which components have finished
        ...
        for thisComponent in presentation_trainingComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        train_digitloopPF.addData('digitpresentation_training.started', digitpresentation_training.tStartRefresh)
        train_digitloopPF.addData('digitpresentation_training.stopped', digitpresentation_training.tStopRefresh)
        thisExp.nextEntry()
        
    # completed digitSpan_trainPF repeats of 'train_digitloopPF'
    ...
    for thisComponent in recall_trainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trial_trainingPF.addData('recall_train.started', recall_train.tStartRefresh)
    trial_trainingPF.addData('recall_train.stopped', recall_train.tStopRefresh)
    trial_trainingPF.addData('answ_train.text',answ_train.text)
    trial_trainingPF.addData('answ_train.started', answ_train.tStartRefresh)
    trial_trainingPF.addData('answ_train.stopped', answ_train.tStopRefresh)
    trial_trainingPF.addData('next_train.numClicks', next_train.numClicks)
    if next_train.numClicks:
       trial_trainingPF.addData('next_train.timesOn', next_train.timesOn)
       trial_trainingPF.addData('next_train.timesOff', next_train.timesOff)
    else:
       trial_trainingPF.addData('next_train.timesOn', "")
       trial_trainingPF.addData('next_train.timesOff', "")
    if answ_train.text == str(answer_trainPF):
        correct = 1
        fbTxt = 'Correct!'
    else:
        correct = 0
...
# completed 1.0 repeats of 'trial_trainingPF'
...
for thisTrial_PF in trial_PF:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    startPF.setSound('datasets/microwave-beep.wav', secs=0.5, hamming=True)
    startPF.setVolume(1.0, log=False)
    first_fix.setText('+')
    # keep track of which components have finished
    first_fixationComponents = [driftPF, startPF, first_fix]
    ...
    while continueRoutine:
        ...
        # *driftPF* updates
        if driftPF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            driftPF.frameNStart = frameN  # exact frame index
            driftPF.tStart = t  # local t and not account for scr refresh
            driftPF.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(driftPF, 'tStartRefresh')  # time at next scr refresh
            driftPF.setAutoDraw(True)
        if driftPF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > driftPF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                driftPF.tStop = t  # not accounting for scr refresh
                driftPF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(driftPF, 'tStopRefresh')  # time at next scr refresh
                driftPF.setAutoDraw(False)
        # start/stop startPF
        if startPF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            startPF.frameNStart = frameN  # exact frame index
            startPF.tStart = t  # local t and not account for scr refresh
            startPF.tStartRefresh = tThisFlipGlobal  # on global time
            startPF.play(when=win)  # sync with win flip
        if startPF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > startPF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                startPF.tStop = t  # not accounting for scr refresh
                startPF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(startPF, 'tStopRefresh')  # time at next scr refresh
                startPF.stop()
        ...
        if first_fix.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > first_fix.tStartRefresh + fixation_time_PF-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in first_fixationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    startPF.stop()  # ensure sound has stopped at end of routine
    trial_PF.addData('first_fix.started', first_fix.tStartRefresh)
    trial_PF.addData('first_fix.stopped', first_fix.tStopRefresh)
    # the Routine "first_fixation" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    digitLoop_PF = data.TrialHandler(nReps=digitSpan_PF, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisDigitLoop_PF in digitLoop_PF:
        ...
        routineTimer.add(1.000000)
        # update component parameters for each repeat
        pres_text.setText(str(digits_PF)[digitLoop_PF.thisN])
        # keep track of which components have finished
        ...
        for thisComponent in PresentationComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        digitLoop_PF.addData('pres_text.started', pres_text.tStartRefresh)
        digitLoop_PF.addData('pres_text.stopped', pres_text.tStopRefresh)
    ...
    trial_PF.addData('answer_box.text',answer_box.text)
    trial_PF.addData('answer_box.started', answer_box.tStartRefresh)
    trial_PF.addData('answer_box.stopped', answer_box.tStopRefresh)
    ...
    if answer_box.text == str(answer_PF):
        correct = 1
        fbTxt = 'Correct!'
    else:
        correct = 0
