...
def show_screen2(disp, screen, log_message, time_status, duration, code): #tracker
    # if log_message is not None: #comentado rl
    ...
    if time_status:
        ...
        disp.fill(screen=screen)
        disp.show()
#    parallel.setPin(3, 0)#comentado RL


def synch_stimuli(disp, pixels_distance): #tracker
    positions = [(960, 140),
                 (960, 940),
                 (560, 540),
                 (1360, 540)]
    fix_start= libscreen.Screen() #comentado RL
    ...
    for i, position in enumerate(positions):
        ...
        # Show fixation COMENTADO POR RL
        # ~ show_screen2(disp, fix_start, 'test_fix_start_' + str(i), time_status=True,
                    # ~ duration=fixation_time_1, code=1) #tracker

        # Show stimuli
        show_screen2(disp, stimuli_screen, 'test_circle_' + str(i), time_status=True,
                    duration=stimulus_time, code=2)#tracker



def experiment_setup(cfg):
    # Parallel port (lo comenté; con esto comentado, funciona)
    ...
    return disp, keyboard, log, instructions_screens, stimuli_screens, fix_screen, stimuli_images, participant #tracker
...
def free_visual_exp_run(cfg):
    # Experiment setup
    disp, keyboard, log, instructions_screens, stimuli_screens, fix_screen, stimuli_images, participant = experiment_setup(cfg=cfg) #tracker (después de disp)
    # print('TRACKER0') #comentado
    # print(tracker.eventdetection) #comentado
    # # Calibrate eye tracker #comentado
    #tracker.calibrate() #comentado
    ...
    # Synch test
    synch_stimuli(disp, pixels_distance) #tracker
    ...
    with open(cfg['results_folder'] + participant + '.csv', 'w') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=['sampling_time', 'likert_1', 'likert_2', 'likert_3', 'stimulus_screen', 'likert_0'], delimiter=';') #'used_saccades', 'total_saccades',
        writer.writeheader()
        ...
        for trial_index, stimulus_screen in enumerate(stimuli_screens):
            ...
            if trial_index == total_trials/2:
                show_screen(disp=disp, screen=rest_screen, log_message='rest_show') #tracker=tracker, antes de log
                keyboard.get_key()
            ...
            fix_screen.draw_rect(colour=WHITE,
                             w=25, h=25, x=1895, y=1055, pw=500, fill=True)
            show_screen(disp=disp, screen=fix_screen, log_message='fixation')#tracker=tracker, antes de log
           ...
           # tracker.status_msg("Trial %d" % trial_index)
            ...
            stimulus_screen.draw_rect(colour=BLACK,
                             w=25, h=25, x=1895, y=1055, pw=500, fill=True)
            show_screen(disp=disp, screen=stimulus_screen,  log_message='stimulus_show') #tracker=tracker, antes de log

            # Stimulus loop #comentado RL
            # stimulus_loop(trial_index=trial_index,disp=disp, screen=stimulus_screen, 
                          # end_condition=True, cfg=cfg, image_file=stimuli_images[trial_index], keyboard=keyboard, 
                          # log=log, saccade_stats=saccade_stats, sampling_time=sampling_time, gabor_image_tuples=gabor_image_tuples) #tracker=tracker, después de screen
            ...
            #ivan lo dejó comentado desde aquí hasta TODO: regex number
    ...
    # End experiment
    show_screen(disp=disp, screen=end_screen, log_message='end_show') #tracker=tracker, 
 ...
 #comenté desde def hasta tracker.log
...
# comenté todo def stimulus loop
...
def show_screen(disp, screen, log_message): #tracker, antes de log
    disp.fill(screen=screen)
    disp.show()
    #if log_message is not None:
        #tracker.log(log_message)

#comenté def drift_correction
