...
expName = 'DigitSpanTask_activeback'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\INU\\Documents\\GitHub\\NeuroCog_CIAE\\TaskTemplates\\PsychoPy_tasks\\digit-span-fwd\\DigitSpanTask_activeback_lastrun.py',
...
Bienvenida_img = visual.ImageStim(
    win=win,
    name='Bienvenida_img', 
    image='Bienvenida.png', mask=None, anchor='center',
...
start_train = sound.Sound('microwave-beep.wav', secs=0.5, stereo=True, hamming=True,
    name='start_train')
start_train.setVolume(1.0)
drift_train = visual.ImageStim(
    win=win,
    name='drift_train', 
    image='drift.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=None,
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
end_training = visual.ImageStim(
    win=win,
    name='end_training', 
    image='finpractica.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=None,
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
start_AF = sound.Sound('microwave-beep.wav', secs=0.5, stereo=True, hamming=True,
    name='start_AF')
start_AF.setVolume(1.0)
drift_AF = visual.ImageStim(
    win=win,
    name='drift_AF', 
    image='drift.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=None,
...
# set up handler to look after randomisation of conditions etc
trial_trainingAF = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('training_af.xlsx'),
    seed=None, name='trial_trainingAF')
thisExp.addLoop(trial_trainingAF)  # add the loop to the experiment
thisTrial_trainingAF = trial_trainingAF.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrial_trainingAF.rgb)
if thisTrial_trainingAF != None:
    for paramName in thisTrial_trainingAF:
        exec('{} = thisTrial_trainingAF[paramName]'.format(paramName))

for thisTrial_trainingAF in trial_trainingAF:
    currentLoop = trial_trainingAF
    # abbreviate parameter names if possible (e.g. rgb = thisTrial_trainingAF.rgb)
    if thisTrial_trainingAF != None:
        for paramName in thisTrial_trainingAF:
            exec('{} = thisTrial_trainingAF[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    start_train.setSound('microwave-beep.wav', secs=0.5, hamming=True)
    ...
    trial_trainingAF.addData('fixation_press_training.started', fixation_press_training.tStartRefresh)
    trial_trainingAF.addData('fixation_press_training.stopped', fixation_press_training.tStopRefresh)
    ...
    while continueRoutine:
        ...
        if firstfixtrain.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > firstfixtrain.tStartRefresh + fixation_time_trainAF-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in firstfix_trainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trial_trainingAF.addData('firstfixtrain.started', firstfixtrain.tStartRefresh)
    trial_trainingAF.addData('firstfixtrain.stopped', firstfixtrain.tStopRefresh)
    # the Routine "firstfix_training" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    train_digitloopAF = data.TrialHandler(nReps=digitSpan_trainAF, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='train_digitloopAF')
    thisExp.addLoop(train_digitloopAF)  # add the loop to the experiment
    thisTrain_digitloopAF = train_digitloopAF.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrain_digitloopAF.rgb)
    if thisTrain_digitloopAF != None:
        for paramName in thisTrain_digitloopAF:
            exec('{} = thisTrain_digitloopAF[paramName]'.format(paramName))
    
    for thisTrain_digitloopAF in train_digitloopAF:
        currentLoop = train_digitloopAF
        # abbreviate parameter names if possible (e.g. rgb = thisTrain_digitloopAF.rgb)
        if thisTrain_digitloopAF != None:
            for paramName in thisTrain_digitloopAF:
                exec('{} = thisTrain_digitloopAF[paramName]'.format(paramName))
        ...
        # update component parameters for each repeat
        digitpresentation_training.setText(str(digits_trainAF)[train_digitloopAF.thisN])
        # keep track of which components have finished
        ...
        for thisComponent in presentation_trainingComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        train_digitloopAF.addData('digitpresentation_training.started', digitpresentation_training.tStartRefresh)
        train_digitloopAF.addData('digitpresentation_training.stopped', digitpresentation_training.tStopRefresh)
        thisExp.nextEntry()
        
    # completed digitSpan_trainAF repeats of 'train_digitloopAF'
    ...
    for thisComponent in recall_trainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trial_trainingAF.addData('recall_train.started', recall_train.tStartRefresh)
    trial_trainingAF.addData('recall_train.stopped', recall_train.tStopRefresh)
    trial_trainingAF.addData('answ_train.text',answ_train.text)
    trial_trainingAF.addData('answ_train.started', answ_train.tStartRefresh)
    trial_trainingAF.addData('answ_train.stopped', answ_train.tStopRefresh)
    trial_trainingAF.addData('next_train.numClicks', next_train.numClicks)
    if next_train.numClicks:
       trial_trainingAF.addData('next_train.timesOn', next_train.timesOn)
       trial_trainingAF.addData('next_train.timesOff', next_train.timesOff)
    else:
       trial_trainingAF.addData('next_train.timesOn', "")
       trial_trainingAF.addData('next_train.timesOff', "")
    if answ_train.text == str(answer_trainAF):
        correct = 1
        fbTxt = 'Correct!'
    else:
        correct = 0
...
# completed 1.0 repeats of 'trial_trainingAF'
...
# set up handler to look after randomisation of conditions etc
trial_AF = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Dataset_AF.xlsx'),
    seed=None, name='trial_AF')
thisExp.addLoop(trial_AF)  # add the loop to the experiment
thisTrial_AF = trial_AF.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrial_AF.rgb)
if thisTrial_AF != None:
    for paramName in thisTrial_AF:
        exec('{} = thisTrial_AF[paramName]'.format(paramName))

for thisTrial_AF in trial_AF:
    currentLoop = trial_AF
    # abbreviate parameter names if possible (e.g. rgb = thisTrial_AF.rgb)
    if thisTrial_AF != None:
        for paramName in thisTrial_AF:
            exec('{} = thisTrial_AF[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    start_AF.setSound('microwave-beep.wav', secs=0.5, hamming=True)
    start_AF.setVolume(1.0, log=False)
    active_press.keys = []
    active_press.rt = []
    _active_press_allKeys = []
    # keep track of which components have finished
    press_fixComponents = [start_AF, drift_AF, press_fixation, active_press]
    ...
    while continueRoutine:
        ...
        # update/draw components on each frame
        # start/stop start_AF
        if start_AF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            start_AF.frameNStart = frameN  # exact frame index
            start_AF.tStart = t  # local t and not account for scr refresh
            start_AF.tStartRefresh = tThisFlipGlobal  # on global time
            start_AF.play(when=win)  # sync with win flip
        if start_AF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > start_AF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                start_AF.tStop = t  # not accounting for scr refresh
                start_AF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(start_AF, 'tStopRefresh')  # time at next scr refresh
                start_AF.stop()
        
        # *drift_AF* updates
        if drift_AF.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            drift_AF.frameNStart = frameN  # exact frame index
            drift_AF.tStart = t  # local t and not account for scr refresh
            drift_AF.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(drift_AF, 'tStartRefresh')  # time at next scr refresh
            drift_AF.setAutoDraw(True)
        if drift_AF.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > drift_AF.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                drift_AF.tStop = t  # not accounting for scr refresh
                drift_AF.frameNStop = frameN  # exact frame index
                win.timeOnFlip(drift_AF, 'tStopRefresh')  # time at next scr refresh
                drift_AF.setAutoDraw(False)
    ...
    for thisComponent in press_fixComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    start_AF.stop()  # ensure sound has stopped at end of routine
    trial_AF.addData('press_fixation.started', press_fixation.tStartRefresh)
    trial_AF.addData('press_fixation.stopped', press_fixation.tStopRefresh)
    ...
    while continueRoutine:
        ...
        if first_fix.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > first_fix.tStartRefresh + fixation_time_AF-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in first_fixationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trial_AF.addData('first_fix.started', first_fix.tStartRefresh)
    trial_AF.addData('first_fix.stopped', first_fix.tStopRefresh)
    # the Routine "first_fixation" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    digitLoop_AF = data.TrialHandler(nReps=digitSpan_AF, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='digitLoop_AF')
    thisExp.addLoop(digitLoop_AF)  # add the loop to the experiment
    thisDigitLoop_AF = digitLoop_AF.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisDigitLoop_AF.rgb)
    if thisDigitLoop_AF != None:
        for paramName in thisDigitLoop_AF:
            exec('{} = thisDigitLoop_AF[paramName]'.format(paramName))
    
    for thisDigitLoop_AF in digitLoop_AF:
        currentLoop = digitLoop_AF
        # abbreviate parameter names if possible (e.g. rgb = thisDigitLoop_AF.rgb)
        if thisDigitLoop_AF != None:
            for paramName in thisDigitLoop_AF:
                exec('{} = thisDigitLoop_AF[paramName]'.format(paramName))
        ...
        routineTimer.add(1.000000)
        # update component parameters for each repeat
        pres_text.setText(str(digits_AF)[digitLoop_AF.thisN])
        # keep track of which components have finished
        ...
        for thisComponent in PresentationComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        digitLoop_AF.addData('pres_text.started', pres_text.tStartRefresh)
        digitLoop_AF.addData('pres_text.stopped', pres_text.tStopRefresh)
    # completed digitSpan_AF repeats of 'digitLoop_AF'
    ...
    for thisComponent in RecallComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trial_AF.addData('recall_txt.started', recall_txt.tStartRefresh)
    trial_AF.addData('recall_txt.stopped', recall_txt.tStopRefresh)
    trial_AF.addData('answer_box.text',answer_box.text)
    trial_AF.addData('answer_box.started', answer_box.tStartRefresh)
    trial_AF.addData('answer_box.stopped', answer_box.tStopRefresh)
    trial_AF.addData('button_next.numClicks', button_next.numClicks)
    if button_next.numClicks:
       trial_AF.addData('button_next.timesOn', button_next.timesOn)
       trial_AF.addData('button_next.timesOff', button_next.timesOff)
    else:
       trial_AF.addData('button_next.timesOn', "")
       trial_AF.addData('button_next.timesOff', "")
    if answer_box.text == str(answer_AF):
        correct = 1
        fbTxt = 'Correct!'
    else:
        correct = 0
...
# completed 1.0 repeats of 'trial_AF'
