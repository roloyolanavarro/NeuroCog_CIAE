...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\INU\\Documents\\GitHub\\NeuroCog_CIAE\\TaskTemplates\\PsychoPy_tasks\\WorkshopTask\\MyTestTask_lastrun.py',
...
while continueRoutine:
    ...
    if quit_resp.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > quit_resp.tStartRefresh + 1.0-frameTolerance:
            # keep track of stop time/frame for later
            quit_resp.tStop = t  # not accounting for scr refresh
            quit_resp.frameNStop = frameN  # exact frame index
            win.timeOnFlip(quit_resp, 'tStopRefresh')  # time at next scr refresh
            quit_resp.status = FINISHED
...
# check responses
if quit_resp.keys in ['', [], None]:  # No response was made
    quit_resp.keys = None
thisExp.addData('quit_resp.keys',quit_resp.keys)
if quit_resp.keys != None:  # we had a response
    thisExp.addData('quit_resp.rt', quit_resp.rt)
thisExp.nextEntry()
