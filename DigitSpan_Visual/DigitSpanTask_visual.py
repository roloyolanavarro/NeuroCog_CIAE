...
if len(_answ_key_train_allKeys):
    for key in _answ_key_train_allKeys:
        if key.name[0]=="n":
            key.name=key.name[4:]
        print(f'keyname: {key.name}')
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\lroci\\Documents\\GitHub\\NeuroCog_CIAE\\DigitSpanTask\\DigitSpanTask_visual.py',
...
recall_train = visual.TextStim(win=win, name='recall_train',
    ...
    languageStyle='LTR',
    depth=0.0);
screentext_train = visual.TextStim(win=win, name='screentext_train',
    text='',
    font='Arial',
    ...
    languageStyle='LTR',
    depth=-1.0);
answ_key_train = keyboard.Keyboard()
...
recall_txt = visual.TextStim(win=win, name='recall_txt',
    ...
    languageStyle='LTR',
    depth=0.0);
answ_key = keyboard.Keyboard()
screen_text_pas = visual.TextStim(win=win, name='screen_text_pas',
    text='',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
recall_train_2 = visual.TextStim(win=win, name='recall_train_2',
    ...
    languageStyle='LTR',
    depth=0.0);
screentext_train_act = visual.TextStim(win=win, name='screentext_train_act',
    text='',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-1.0);
answ_key_train2 = keyboard.Keyboard()
...
recall_txt_2 = visual.TextStim(win=win, name='recall_txt_2',
    ...
    languageStyle='LTR',
    depth=0.0);
answ_key2 = keyboard.Keyboard()
screen_text_act = visual.TextStim(win=win, name='screen_text_act',
    text='',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
Block_Order = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('datasets/task_order.csv', selection=lasfilas),
...
for thisBlock_Order in Block_Order:
    ...
    for thisBlockPasiva in BlockPasivas:
        ...
        instrucloop_Pasivas = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('datasets/instructions.csv'),
        ...
        trial_trainingPasivas = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions("datasets/training_"+ordenpasivas[BlockPasivas.thisN]+".csv"),
        ...
        for thisTrial_trainingPasiva in trial_trainingPasivas:
            ...
            answ_key_train.keys = []
            answ_key_train.rt = []
            _answ_key_train_allKeys = []
            screen_text_train = ''
            larespuesta = ''
            answer_train_pasivas=int(answer_train)
            
            # keep track of which components have finished
            recall_trainingComponents = [recall_train, screentext_train, answ_key_train]
            ...
            while continueRoutine:
                ...
                # *screentext_train* updates
                if screentext_train.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    screentext_train.frameNStart = frameN  # exact frame index
                    screentext_train.tStart = t  # local t and not account for scr refresh
                    screentext_train.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(screentext_train, 'tStartRefresh')  # time at next scr refresh
                    screentext_train.setAutoDraw(True)
                if screentext_train.status == STARTED:  # only update if drawing
                    screentext_train.setText(screen_text_train, log=False)
                
                # *answ_key_train* updates
                waitOnFlip = False
                if answ_key_train.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    answ_key_train.frameNStart = frameN  # exact frame index
                    answ_key_train.tStart = t  # local t and not account for scr refresh
                    answ_key_train.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(answ_key_train, 'tStartRefresh')  # time at next scr refresh
                    answ_key_train.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(answ_key_train.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(answ_key_train.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if answ_key_train.status == STARTED and not waitOnFlip:
                    theseKeys = answ_key_train.getKeys(keyList=None, waitRelease=False)
                    _answ_key_train_allKeys.extend(theseKeys)
                    if len(_answ_key_train_allKeys):
                        answ_key_train.keys = [key.name for key in _answ_key_train_allKeys]  # storing all keys
                        answ_key_train.rt = [key.rt for key in _answ_key_train_allKeys]
                screen_text_train = ''.join(answ_key_train.keys)
                
                if ("return" in answ_key_train.keys):
                    answ_key_train.keys.remove("return")
                    larespuesta = ''.join(map(str,answ_key_train.keys))
                    print(larespuesta)
                    
                    if larespuesta == str(answer_train_pasivas):
                        correct = 1
                    else:
                        correct = 0
                    thisExp.addData('correct', correct)
                    thisExp.addData("recall", larespuesta)
                    continueRoutine = False
            ...
            trial_trainingPasivas.addData('screentext_train.started', screentext_train.tStartRefresh)
            trial_trainingPasivas.addData('screentext_train.stopped', screentext_train.tStopRefresh)
            # check responses
            if answ_key_train.keys in ['', [], None]:  # No response was made
                answ_key_train.keys = None
            trial_trainingPasivas.addData('answ_key_train.keys',answ_key_train.keys)
            if answ_key_train.keys != None:  # we had a response
                trial_trainingPasivas.addData('answ_key_train.rt', answ_key_train.rt)
            trial_trainingPasivas.addData('answ_key_train.started', answ_key_train.tStartRefresh)
            trial_trainingPasivas.addData('answ_key_train.stopped', answ_key_train.tStopRefresh)
            ...
            # update component parameters for each repeat
            if correct==0:
                feedbackpas= f'la respuesta correcta era {answer_train_pasivas}'
            else:
                feedbackpas= f'Tu respuesta es correcta'
            
            post_recall_training.setText(feedbackpas)
            # keep track of which components have finished
        ...
        trial_Pasiva = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions("datasets/Dataset_"+ordenpasivas[BlockPasivas.thisN]+".csv"),
        ...
        for thisTrial_Pasiva in trial_Pasiva:
            ...
            # keep track of which components have finished
            Recall_pasivasComponents = [recall_txt, answ_key, screen_text_pas]
            ...
            while continueRoutine:
                ...
                # *answ_key* updates
                waitOnFlip = False
                if answ_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    answ_key.frameNStart = frameN  # exact frame index
                    answ_key.tStart = t  # local t and not account for scr refresh
                    answ_key.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(answ_key, 'tStartRefresh')  # time at next scr refresh
                    answ_key.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(answ_key.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(answ_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if answ_key.status == STARTED and not waitOnFlip:
                    theseKeys = answ_key.getKeys(keyList=None, waitRelease=False)
                    _answ_key_allKeys.extend(theseKeys)
                    if len(_answ_key_allKeys):
                        answ_key.keys = [key.name for key in _answ_key_allKeys]  # storing all keys
                        answ_key.rt = [key.rt for key in _answ_key_allKeys]
                
                # *screen_text_pas* updates
                if screen_text_pas.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    screen_text_pas.frameNStart = frameN  # exact frame index
                    screen_text_pas.tStart = t  # local t and not account for scr refresh
                    screen_text_pas.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(screen_text_pas, 'tStartRefresh')  # time at next scr refresh
                    screen_text_pas.setAutoDraw(True)
                if screen_text_pas.status == STARTED:  # only update if drawing
                    screen_text_pas.setText(screen_text, log=False)
                screen_text = ''.join(answ_key.keys)
                
                if ("return" in answ_key.keys): #end routine when return is typed
                    answ_key.keys.remove("return") #deletes return from answer
                    larespuesta = ''.join(map(str,answ_key.keys)) #turn list into string
                    print(larespuesta)
                    if larespuesta == str(answer_Pasive): #if correct
                        correct = 1
                    else:
                        correct = 0
                    thisExp.addData('correct', correct)
            ...
            # check responses
            if answ_key.keys in ['', [], None]:  # No response was made
                answ_key.keys = None
            trial_Pasiva.addData('answ_key.keys',answ_key.keys)
            if answ_key.keys != None:  # we had a response
                trial_Pasiva.addData('answ_key.rt', answ_key.rt)
            trial_Pasiva.addData('answ_key.started', answ_key.tStartRefresh)
            trial_Pasiva.addData('answ_key.stopped', answ_key.tStopRefresh)
            trial_Pasiva.addData('screen_text_pas.started', screen_text_pas.tStartRefresh)
            trial_Pasiva.addData('screen_text_pas.stopped', screen_text_pas.tStopRefresh)
            ###trial 1 or 2 (span 3)
    ...
    for thisBlockActiva in BlockActivas:
        ...
        trial_train_Activas = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions("datasets/training_"+ordenactivas[BlockActivas.thisN]+".csv"),
        ...
        for thisTrial_train_Activa in trial_train_Activas:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            answ_key_train2.keys = []
            answ_key_train2.rt = []
            _answ_key_train2_allKeys = []
            screen_text_train = ''
            larespuesta = ''
            answer_train_activas=int(answer_train)
            # keep track of which components have finished
            recall_train_activasComponents = [recall_train_2, screentext_train_act, answ_key_train2]
            ...
            while continueRoutine:
                ...
                # *screentext_train_act* updates
                if screentext_train_act.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    screentext_train_act.frameNStart = frameN  # exact frame index
                    screentext_train_act.tStart = t  # local t and not account for scr refresh
                    screentext_train_act.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(screentext_train_act, 'tStartRefresh')  # time at next scr refresh
                    screentext_train_act.setAutoDraw(True)
                if screentext_train_act.status == STARTED:  # only update if drawing
                    screentext_train_act.setText(screen_text_train, log=False)
                
                # *answ_key_train2* updates
                waitOnFlip = False
                if answ_key_train2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    answ_key_train2.frameNStart = frameN  # exact frame index
                    answ_key_train2.tStart = t  # local t and not account for scr refresh
                    answ_key_train2.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(answ_key_train2, 'tStartRefresh')  # time at next scr refresh
                    answ_key_train2.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(answ_key_train2.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(answ_key_train2.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if answ_key_train2.status == STARTED and not waitOnFlip:
                    theseKeys = answ_key_train2.getKeys(keyList=None, waitRelease=False)
                    _answ_key_train2_allKeys.extend(theseKeys)
                    if len(_answ_key_train2_allKeys):
                        answ_key_train2.keys = [key.name for key in _answ_key_train2_allKeys]  # storing all keys
                        answ_key_train2.rt = [key.rt for key in _answ_key_train2_allKeys]
                screen_text_train = ''.join(answ_key_train2.keys)
                    
                if("return" in answ_key_train2.keys):
                    answ_key_train2.keys.remove("return")
                    larespuesta = ''.join(map(str,answ_key_train2.keys))
                    print(larespuesta)
                    if larespuesta == str(answer_train_activas):
                        correct = 1
                    else:
                        correct = 0
                    thisExp.addData('correct', correct)
            ...
            trial_train_Activas.addData('screentext_train_act.started', screentext_train_act.tStartRefresh)
            trial_train_Activas.addData('screentext_train_act.stopped', screentext_train_act.tStopRefresh)
            # check responses
            if answ_key_train2.keys in ['', [], None]:  # No response was made
                answ_key_train2.keys = None
            trial_train_Activas.addData('answ_key_train2.keys',answ_key_train2.keys)
            if answ_key_train2.keys != None:  # we had a response
                trial_train_Activas.addData('answ_key_train2.rt', answ_key_train2.rt)
            trial_train_Activas.addData('answ_key_train2.started', answ_key_train2.tStartRefresh)
            trial_train_Activas.addData('answ_key_train2.stopped', answ_key_train2.tStopRefresh)
            ...
            # update component parameters for each repeat
            if correct==0:
                feedbackact= f'la respuesta correcta era {answer_train_activas}'
            else:
                feedbackact= f'Tu respuesta es correcta'
            
            post_recall_training_2.setText(feedbackact)
            # keep track of which components have finished
        ...
        trialActivas = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions("datasets/Dataset_"+ordenactivas[BlockActivas.thisN]+".csv"),
        ...
        for thisTrialActiva in trialActivas:
            ...
            screen_text = ''
            # keep track of which components have finished
            RecallActivasComponents = [recall_txt_2, answ_key2, screen_text_act]
            ...
            while continueRoutine:
                ...
                # *answ_key2* updates
                waitOnFlip = False
                if answ_key2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    answ_key2.frameNStart = frameN  # exact frame index
                    answ_key2.tStart = t  # local t and not account for scr refresh
                    answ_key2.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(answ_key2, 'tStartRefresh')  # time at next scr refresh
                    answ_key2.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(answ_key2.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(answ_key2.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if answ_key2.status == STARTED and not waitOnFlip:
                    theseKeys = answ_key2.getKeys(keyList=None, waitRelease=False)
                    _answ_key2_allKeys.extend(theseKeys)
                    if len(_answ_key2_allKeys):
                        answ_key2.keys = [key.name for key in _answ_key2_allKeys]  # storing all keys
                        answ_key2.rt = [key.rt for key in _answ_key2_allKeys]
                
                # *screen_text_act* updates
                if screen_text_act.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    screen_text_act.frameNStart = frameN  # exact frame index
                    screen_text_act.tStart = t  # local t and not account for scr refresh
                    screen_text_act.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(screen_text_act, 'tStartRefresh')  # time at next scr refresh
                    screen_text_act.setAutoDraw(True)
                if screen_text_act.status == STARTED:  # only update if drawing
                    screen_text_act.setText(screen_text, log=False)
                screen_text = ''.join(answ_key2.keys)
                
                if("return" in answ_key2.keys):  #end routine when return is typed
                    answ_key2.keys.remove("return") #deletes return from answer
                    larespuesta = ''.join(map(str,answ_key2.keys))
                    print(larespuesta)
                    if larespuesta == str(answer_active): #if correct
                        correct = 1
                    else:
                        correct = 0
                    thisExp.addData('correct', correct)
            ...
            # check responses
            if answ_key2.keys in ['', [], None]:  # No response was made
                answ_key2.keys = None
            trialActivas.addData('answ_key2.keys',answ_key2.keys)
            if answ_key2.keys != None:  # we had a response
                trialActivas.addData('answ_key2.rt', answ_key2.rt)
            trialActivas.addData('answ_key2.started', answ_key2.tStartRefresh)
            trialActivas.addData('answ_key2.stopped', answ_key2.tStopRefresh)
            trialActivas.addData('screen_text_act.started', screen_text_act.tStartRefresh)
            trialActivas.addData('screen_text_act.stopped', screen_text_act.tStopRefresh)
            ###trial 1 or 2 (span 3)
