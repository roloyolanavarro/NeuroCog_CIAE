...
ONE_CM_TO_PIXELS = 65.35
ONE_DEGREE_CM_TO_SCREEN_EQUIVALENCE = 57.0


def degrees_to_pixels_transformation(distance_from_screen, degrees):
...
pixels_screen = degrees_to_pixels_transformation(distance_from_screen=70, degrees=46)
print(str(pixels_screen))
...
targetscreens['draw_on_saccade'] = libscreen.Screen()
feedbackscreens = {}
...
for trialnr in range(1, 21):
    ...
    for i in range(20):
        ...
        # present feedback
        if i % 5 == 0:
            targetscreens['draw_on_saccade'].draw_circle(pos=endpos,
                                                         fill=True)

        disp.fill(targetscreens['draw_on_saccade'])
        disp.show()
        libtime.pause(500)
    # stop eye tracking
